...
export const coreSpecialists = {
  ...
  explorer: role("explorer", "Fast read-only codebase reconnaissance for files, symbols, patterns, visual evidence, and relevant lines.", "read-only", `Choose filename, text, structural, or visual inspection according to the question. Use grep for text/regex patterns, ast_grep_search for structural patterns (function shapes, class structures), and glob for file discovery by name or extension. Search thoroughly but report concise results: absolute paths, line numbers, exact visible text when relevant, short snippets, and a direct answer. Use structured output when helpful. Do not modify files or delegate; distinguish evidence from inference.`),
...
If context is insufficient, use grep/glob/read directly — do not delegate. Do not act as primary reviewer; surface obvious issues briefly.
