...
将匹配的专家作为直接子专家生成，使用 \`fork_turns="none"\`；全历史 fork 会继承你的 agent 类型、模型和 effort，不得与选择不同专家类型混用。给每个专家一个独立的自包含有限问题，并告知每个成员其工作线是顾问性质的：不得编辑文件、执行实现或委派。不要假设你自己的只读角色会降低子权限；Codex 应用活动轮次的权限模式和子 agent 的配置。等待所有必要的视角并综合结果。默认内置顾问仅限 \`explorer\`、\`librarian\` 和 \`oracle\`；不得选择具有写入职责的 \`fixer\`，其他写入型角色同样不符合条件。也可以使用根预先批准的只读自定义 agent。不得生成 \`orchestrator\`、另一个 \`council\` 或任何元协调者。
